...
let getSkyColor time : Color =
  if time < 6.0f then
    ...
    lerpColor (Color(10, 10, 30)) (Color(100, 149, 237)) ((time - 6.0f) / 2.0f)
    ...
    lerpColor (Color(50, 50, 100)) (Color(10, 10, 30)) ((time - 18.0f) / 2.0f)
  else
    Color(10, 10, 30)
...
let getAmbientColor time : Color =
  if time < 5.0f || time > 19.0f then
    ...
    let r = int(15.0f + t * 80.0f)
    let g = int(20.0f + t * 100.0f)
    let b = int(45.0f + t * 110.0f)
    Color(r, g, b)
  elif time < 17.0f then
    Color(95, 130, 155)
  elif time < 19.0f then
    let t = (time - 17.0f) / 2.0f
    let r = int(95.0f + t * 40.0f)
    let g = int(130.0f + t * 50.0f)
    let b = int(155.0f + t * 60.0f)
    Color(r, g, b)
  else
    Color(40, 50, 120)
...
/// Returns the primary directional light direction for the current time of day.
/// Sun: 6h–18h, Moon: 18h–6h, with a smooth arc across the sky.
let getPrimaryLightDirection (time: float32) (arcRadius: float32) : Vector3 =
  if time >= 6.0f && time <= 18.0f then
    // Sun: t=0 at 6h, t=1 at 18h.
    celestialArc ((time - 6.0f) / 12.0f) arcRadius
  else
    // Moon: t=0 at 18h, t=1 at 6h.
    let t =
      if time > 18.0f then
        (time - 18.0f) / 12.0f
      else
