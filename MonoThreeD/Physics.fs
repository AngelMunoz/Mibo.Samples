...
let resolveCollision
  ...
  let _ = (prevPos, playerBottom prevPos) // prevPlayerBottomY kept for parity/readability
  ...
  for KeyValue(struct (cx, cz), chunk) in chunks do
    if abs(cx - pcx) <= 2 && abs(cz - pcz) <= 2 then
      // Grid origin is stored in System.Numerics form; convert the floats to XNA.
