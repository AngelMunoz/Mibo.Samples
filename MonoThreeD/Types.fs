...
[<Struct>]
type Chunk = {
  // CellGrid3D is a Core (backend-agnostic) type; it stores its Origin/CellSize as
  // System.Numerics.Vector3, but those are pure float triples — we keep the grid
  // in Numerics form internally and convert at the renderer boundary (View.fs).
...
type MinimapModel(playerPos: Vector3) =
  ...
  // CPU backing buffer for the minimap texture (MonoGame has no raylib-style Image
  // CPU API; we blit into this Color[] and upload via Texture2D.SetData).
  member val PixelBuffer = Array.zeroCreate<Color>(0) with get, set

type DiagnosticsModel() =
  // MonoGame has no raylib-style default font; the diagnostics font is loaded
  // from the content pipeline (Content/diagnostics.spritefont → "diagnostics").
  member val Font = Unchecked.defaultof<SpriteFont> with get, set
...
// MonoGame uses the AnimatedModel type (carries Model + Mesh + State together),
// whereas the Raylib sample used Animation3DState (which carries a .Model field).
type GameModel() =
