...
let view (ctx: GameContext) (model: Model) (buffer: RenderBuffer2D) =
  ...
  if sunIntensity > 0.0f then
    let sunDir =
      let toSun = Vector2(playerCenterX, groundLevel - 200.0f) - sunPos
      Vector2.Normalize(toSun)
  ...
  if moonIntensity > 0.0f then
    let moonDir =
      let toMoon = Vector2(playerCenterX, groundLevel - 200.0f) - moonPos
      Vector2.Normalize(toMoon)
