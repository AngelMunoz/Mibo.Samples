...
open MonoClient.Types
open MonoClient.NetworkService
open MonoClient.View
open FSharp.UMX
...
let update env msg model : struct (Model * Cmd<_>) =
    ...
    | LocalInput mouseY ->
        ...
        | ValueSome side ->
            ...
            { model with LocalState = localState }, Network.send env.Network (MovePaddle(side, mouseY))
...
let subscribe
    ...
    let msgSub =
        Sub.Active(
            ...
            fun dispatch ->
                transport.MessageReceived.Subscribe(fun bytes -> deserializeGameState bytes |> ServerState |> dispatch)
        )
    ...
    let program =
        ...
        |> Program.withConfig (fun cfg ->
            { cfg with
                ...
                Height = 800
                TargetFPS = 60 })
        |> Program.withRenderer (fun () -> Renderer2D.create (fun c m b -> view c m.LocalState b))
