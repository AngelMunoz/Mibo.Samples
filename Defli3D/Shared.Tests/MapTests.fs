  ...
  testList "Map" [
    ...
    testCase "waypoint centers sit at cell centers" (fun () ->
      for p in map.Path do
        // Origin (0,0), 1-unit cells: centers are at x.5 offsets.
        Expect.equal (p.X % 1f) 0.5f "center x"
        Expect.equal (p.Y % 1f) 0.5f "center y")
    ...
    testCase "cellAt picks the CONTAINING cell (no half-tile shift)" (fun () ->
      ...
      // World (1.5, 1.3) is inside cell (1,1) — including its right and
      // bottom halves.
      match Application.cellAt (Vector2(1.5f, 1.3f)) terrain with
      | ValueSome struct (1, 1) -> ()
      | other -> failtestf "expected (1,1), got %A" other

      match Application.cellAt (Vector2(1.99f, 1.99f)) terrain with
      | ValueSome struct (1, 1) -> ()
      | other -> failtestf "expected (1,1), got %A" other

      // Out of grid → ValueNone.
      Expect.isTrue
        (Application.cellAt (Vector2(-1f, 0f)) terrain).IsNone
        "negative x"

      Expect.isTrue
        (Application.cellAt (Vector2(2000f, 2000f)) terrain).IsNone
        "past the far edge")
        ...
        CellGrid2D.iter
          (fun x y tile ->
            ...
            | ValueSome model ->
              ...
              // Props vs blends: the blend family (detail-dirt) hugs
              // the road; everything else is a scattered prop (both
              // keep Buildable = true on the hand-authored map — the
              // prop is visual only; only procedural obstacles clear
              // it).
              let isBlend =
                model.Name = Models.detailDirt.Name
                || model.Name = Models.detailDirtLarge.Name

              if isBlend then blends <- blends + 1 else props <- props + 1

              // The model is a real baked entry.
              Expect.isTrue (Models.tryByName model.Name).IsSome "baked model"
            | ValueNone -> ())
          deco
  ...
  testList "Map (procedural)" [
    testCase "road is continuous spawn → base, clear of obstacles" (fun () ->
      ...
      for i in 0 .. pmap.Path.Length - 2 do
        ...
        Expect.equal d 1 "one cell step")
