...
// ─────────────────────────────────────────────────────────────
// Systems — the windowed composition root (the MVU shell's
...
module Application =
  ...
  /// The windowed program: input wiring and the per-frame shell phase
  /// over the same sim (Router.step). The raylib client additionally
  /// forces Point filtering on the tilesheet once at boot; the
  /// MonoGame client sets the PointClamp sampler state per frame in
  /// WorldView.worldView instead (no texture-level sampler mutation).
  let windowedProgram
    (cell: WorldCell)
    (shell: Shell)
