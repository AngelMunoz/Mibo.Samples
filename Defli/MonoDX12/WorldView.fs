...
module WorldView =
  ...
  /// MonoGame sampler defaults to LinearClamp; the gutterless atlas
  /// needs Point sampling so adjacent tiles don't bleed into each
  /// other (the raylib client applies the same filter once at boot).
  let worldView
    (shell: Shell)
    (vfx: VfxView)
    (ctx: GameContext)
